...
const productCtrl = {
  ...
  createProducts: async (req, res) => {
    try {
      ...
      res.status(200).json({msg: 'Created product successful!', newProduct})
    } catch (err) {
      return res.status(500).json({ msg: err.message })
    }
  },
  deleteProducts: async (req, res) => {
    try {
      ...
      if (!product) return res.status(400).json('Cannot find product')
  ...
  updateProducts: async (req, res) => {
    try {
      ...
      if (!product) return res.status(500).json({ msg: 'Cannot find product' })
